      ...
      >
        ...
        >
          <img
            ...
            src={`/images/${data?.data.data?.type === "seed" ? "winprice" : "winprice2"
              }.png`}
            alt=""
          ></img>
          <div className="text-center pb-[26px]">
            <p className="mb-2 text-[32px] font-extrabold">
              {`${data?.data.data?.type === "seed" ? "+" : "x"}${data?.data.data.name
                }`}
            </p>
            {
            ...
            <Button
              onClick={handleClose}
              className={clsx(
                " capitalize text-[#fff] capitalize w-full bg-gradient-to-r from-[#97C35B] to-[#61A700]  border-[3px] border-solid border-[#B0D381] drop-shadow-[0_4px_1px_#4C7E0B] ",
