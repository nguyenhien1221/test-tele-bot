...
interface ModalProps {
  handleClose: () => void;
  handleOpenBox: (value: string | null) => void;
}
const CaptChaModal = ({ handleClose, handleOpenBox }: ModalProps) => {
  ...
  return (
    ...
    >
      <div
        className={clsx(
          "h-screen w-screen overflow-hidden flex flex-col flex-1 px-4 pb-10"
        )}
      >
        <div
          className={clsx(
        ...
        >
          ...
          <form>
            <HCaptcha
              ref={captchaRef}
              sitekey={
                process.env.REACT_APP_CAPCHA_SITE_KEY ||
                "042e3498-aa16-4ba8-9cba-75785aa0714a"
              }
              onVerify={(token) => {
                handleOpenBox(token);
              }}
